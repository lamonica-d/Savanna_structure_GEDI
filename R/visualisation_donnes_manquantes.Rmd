...
```{r}
# Visualisation des données manquantes de canopy_cover
...
plot <- ggplot(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  # Customize plot labels and title
  labs(title = "Pourcentage de données manquantes canopy_cover", x = "X-axis Label", y = "% données manquantes") +
  ylim(0,1)
...
```{r}
# Visualisation des données manquantes
# de feu ET de pluie ET de canopy_cover simultanément
...
plot <- ggplot(
               ...
               theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
               labs(title = "Pourcentage de données manquantes feu + pluie + canopy_cover",
               x = "X-axis Label", y = "% données manquantes") +
               ylim(0,1)
...
plot <- ggplot(
               ...
               theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
               labs(
               title = "Pourcentage de données manquantes feu + pluie + canopy_cover",
               x = "X-axis Label",
               y = "% données manquantes"
               ) +
               ylim(0, 1)
...
# Pour ordonner par valeur manquante de canopy_cover
missing_data_percentages <- missing_data_percentages[order(missing_data_percentages$canopy_cover),]
...
# ggsave(
#        "NA_fire_freq_and_mean_precip_sorted_by_canopy_cover_NA.png",
#        plot = plot,
#        bg='#ffffff'# pour ne pas avoir un fond gris moche
#        )
...
# Pour ordonner par valeur manquante de canopy_cover
missing_data_percentages <- missing_data_percentages[order(missing_data_percentages$mean_precip),]
...
# ggsave(
#        "NA_fire_freq_and_mean_precip_sorted_by_mean_precip_NA.png",
#        plot = plot,
#        bg='#ffffff'# pour ne pas avoir un fond gris moche
#        )
