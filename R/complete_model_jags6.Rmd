...
vals["K_G_t_f_pluie_max",] = c(19.4,20,22,24)
...
saveRDS(jags_res,
        file.path(
                  path_to_Savanna_structure_GEDI_folder,
                  "JAGS_outputs",
                  paste0("new_pop_10",Sys.time(),".RDS")
                  )
        )
...
mcmc <- readRDS(
                file.path(
                          path_to_Savanna_structure_GEDI_folder,
                          "JAGS_outputs",
                          "new_pop_102024-07-05 11:25:25.401709.RDS"
                          )
                )
...
##################################################################################
output_file <- file.path(path_to_Savanna_structure_GEDI_folder,"JAGS_outputs",paste(Sys.time(),".txt"))

writeLines(c( readLines(model_txt) , "Résultats :", summary_result ), output_file)
...
pdf(file.path(path_to_Savanna_structure_GEDI_folder,"JAGS_outputs",paste(Sys.time(),"resimulation.pdf")))
...
new_table = as.data.frame(
                         ...
                         table_region$y_TRUE,
                         mean_simu_feu-table_region$fire_freq,
                         table_region,
                         mean_simu_feu,
                         mean_simu_grass,
                         )
