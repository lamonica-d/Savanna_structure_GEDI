...
for (name in names){
  ...
  rm(complete_table)
  x <- cbind(x$x, x$y, x, index = 1:nrow(x))
  x_new <- data.frame(coordx = x$x, coordy = x$y,index = 1:nrow(x))
  ...
  z <- terra::rasterize(x_new_spatvector, y, fun=sample, size = 1, field = "index")
  # fun = sample , size = 1 to get one random sample in the cell
  
  #PROBLEME ICI, RECUP LES COORDONNEES DE LA GRILLE Y
  #trsf_data <- merge(x,data.frame(index = z$index, crds(y)), by = "index")
  ...
  colnames(conserved_sub_table)[4] = "x_center_cell"
  colnames(conserved_sub_table)[5] = "y_center_cell"
  
  # print("colnames(conserved_sub_table)")
  # print(colnames(conserved_sub_table))
  
  conserved_sub_table <- conserved_sub_table[
                        complete.cases(conserved_sub_table[,c("index",
                                                              "x_TRUE",
                                                              "y_TRUE",
                                                              "x_center_cell",
                                                              "y_center_cell",
