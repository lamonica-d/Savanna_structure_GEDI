...
for (name in names){
  ...
  rm(complete_table)
  x <- cbind(x$x, x$y, x, index_point = 1:nrow(x))
  x_new <- data.frame(coordx = x$x, coordy = x$y,index_point = 1:nrow(x))
  ...
  x_new_spatvector <- terra::vect(x_new, geom = c("coordx", "coordy"), crs = "+proj=longlat +datum=WGS84") 
  # get x "window" : xmin, xmax, ymin, ymax
  x_ext <- terra::ext(x_new_spatvector)
  ...
  #2) resampling
  y <- terra::rast(x_new_spatvector, ncol = target_ncol, nrow = target_nrow)
  z <- terra::rasterize(x_new_spatvector, y, fun=sample, size = 1, field = "index_point")
  # fun = sample , size = 1 to get one random sample in the cell
  
  #PROBLEME ICI, RECUP LES COORDONNEES DE LA GRILLE Y
  z2 <- data.frame(cbind(crds(y), values(z)))

  trsf_data <- merge(x[,-c(3:4)],z2, by = "index_point")
  ...
  colnames(conserved_sub_table)[ncol(conserved_sub_table)-1] = "x_center_cell"
  colnames(conserved_sub_table)[ncol(conserved_sub_table)] = "y_center_cell"
  
  # print("colnames(conserved_sub_table)")
  # print(colnames(conserved_sub_table))
  
  conserved_sub_table <- conserved_sub_table[
                        complete.cases(conserved_sub_table[,c( "x_TRUE",
                                                              "y_TRUE",
                                                              "rh98",
                                                              "canopy_cover",
                                                              "fire_freq",
