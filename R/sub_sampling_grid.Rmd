...
for (name in names){
  ...
  #2) resampling
  y <- terra::rast(new_spatvector, ncol = target_ncol, nrow = target_nrow)
 ...
 # NA's   :29001    
  ...
  intermediate_table = data.frame(cbind(crds(y), values(z)))
  colnames(intermediate_table)[1] = "x_center_cell"
  colnames(intermediate_table)[2] = "y_center_cell"
  ...
  for (s in 1:length(conserved)){
    if(s%%2 ==0
      #conserved[s] == TRUE
       ){conserved_indices[s] = s}
  }
  
  # print("conserved_indices")
  # print(conserved_indices[1:1000])
  
  # On ne supprime les indices qu'ici et pas avant pour garder l'ordre des indices des lignes !
  conserved_indices <- conserved_indices[!is.na(conserved_indices)]
  # onserved_indices = 1:nrow(trsf_data) # no selection if this line is unquote
