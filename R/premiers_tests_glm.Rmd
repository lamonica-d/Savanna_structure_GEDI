...
```{r}
# Regardons si les bornes sont atteintes pour tous les jeux de données :
...
# for (i in 1:length(dir())){
  ...
  df_canopy_cover[i,] <- c(
                           ...
                           round(max(corresponding_table[,"canopy_cover"]),3),
  ...
  # we already get rid of the canopy_cover NA

  if(
    is.null(nrow(corresponding_table[,"canopy_cover"]))==FALSE # s'il y a des données
  ){plot(hist(corresponding_table[,"canopy_cover"],breaks=40,main=name,xlab="canopy_cover"))}
  else{plot(c(0,0),main="que des NA pour canopy_cover",xlab="canopy_cover")}
  
  corresponding_table2 <- corresponding_table[complete.cases(corresponding_table[ , "fire_freq"]),]
  # get rid of the fire_freq NA
    
  if(
     is.null(nrow(corresponding_table2[,"fire_freq"]))==FALSE # s'il y a des données
    ){plot(hist(corresponding_table2[,"fire_freq"],breaks=40,main=name,xlab="fire_freq"))}
  else{plot(c(0,0),main="que des NA pour le feu",xlab="fire_freq")}
  
  corresponding_table3 <- corresponding_table[complete.cases(corresponding_table[ , "mean_precip"]),]
  # get rid of the mean_precip NA
  
  if(
     is.null(nrow(corresponding_table3[,"mean_precip"]))==FALSE # s'il y a des données
    ){plot(hist(corresponding_table3[,"mean_precip"],breaks=40,main=name,xlab="mean_precip"))}
