...
vals <- data.frame(matrix(nrow=6,ncol=4))
...
jags_res=foreach(i =1:4,.combine='mcmc.list',.multicombine=TRUE)%dopar%{
  as.mcmc(run.jags(file.path(path_to_Savanna_structure_GEDI_folder,"R","JAGS_models","model1.txt"),monitor=variables,data=data,n.chains=1,inits=dump.format(inits[[i]]),
              burnin=1000,sample=2000,thin=10,tempdir=FALSE,
...
mcmc <- readRDS(
                file.path(
                  path_to_Savanna_structure_GEDI_folder,
                  "JAGS_outputs",
                  "2024-05-15 20:24:59.033691 .RDS")
                )
```
...
K_G_t_f_pluie_max  = 20 # summary(mcmc)$statistics["K_G_t_f_pluie_max",1]
log10a             = summary(mcmc)$statistics["log10a",1]
...
saveRDS(simulations_grass,
        file.path(
                  path_to_Savanna_structure_GEDI_folder,
                  "JAGS_outputs",
                  paste0("simulations_grass",Sys.time(),".RDS"))
        )
...
saveRDS(simulations_feu,
        file.path(
                  path_to_Savanna_structure_GEDI_folder,
                  "JAGS_outputs",
                  paste0("simulations_feu",Sys.time(),".RDS"))
        )
```
