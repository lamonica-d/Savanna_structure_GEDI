...
for (name in names){
  ...
  if(name=="all_Africa"){specific_table <- readRDS(
  ...
  custom_sample <- function(x){
    return(sample(x=x,size=length(x),replace=FALSE))
  }

  #z <- terra::rasterize(new_spatvector, y, fun=sample, size = 1, field = "index_point")
  z <- terra::rasterize(new_spatvector, y, fun=custom_sample, field = "index_point")
  ...
  # Donc à ce stade, on a conservé dans conserved_sub_table uniquement un individu de specific_table par case impaire
  # gardons tous les points qui sont dans un rayon de sqrt(2)*cell des (x_center_cell,y_center_cell) :
  
  final = matrix(ncol=ncol(specific_table))
  colnames(final) = colnames(specific_table)
  
  # 10**3 * 10**6 calcul de distances :
  # N = nrow(specific_table)
  # 
  #  for(i in 1:N){
  #   print(paste("i = ",i,"over",N))
  #   for(u in 1:nrow(conserved_sub_table)){
  #     if(geodist::geodist(
  #       x = specific_table[i,c("x_TRUE","y_TRUE")],
  #       y = conserved_sub_table[u,c("x_center_cell","y_center_cell")],
  #       quiet = TRUE)<sqrt(2)*10**4)
  #       {final = rbind(final,specific_table[i,])}
  #     }
  # } bien trop looooong,b ; 10**6 secondes :D
  
  conserved_sub_table <- subset(conserved_sub_table, select = -c(keep) )
  ...
  saveRDS(
          ...
          file = file.path(
                           path_to_Savanna_structure_GEDI_folder,
                           "transformed_data",
                           paste0(name,"ilots",".RDS")
                           )
          )    
  
  print(paste(paste0(name,"ilots",".RDS"),"DONE"))
