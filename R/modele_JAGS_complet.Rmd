...
```{r model, echo=FALSE, include = FALSE}
 # model and jags inference
  ...
  for (i in 1:N){
      ...
      fire_data[i] ~ dnorm(grassB[i], 1/0.01)T(0,1) #dnorm(grassB[i]**2/(grassB[i]**2 + pt_inflexion_feu**2) , 1/0.01)T(0,1)
      
      }
  
  # priors
  
  pt_inflexion_grass ~ dnorm(800,1/4000) #dnorm(800,1/40000)
  #pt_inflexion_feu ~ dnorm(1,1/4)T(0,)
  K_G_t_f_pluie_max <- 1 #~ dnorm(15,1/8)
  lambda ~ dunif(0.00010,0.0088) #dunif(0.0017,0.0088)
...
inits <- list(
               ...
               list(
             #    K_G_t_f_pluie_max = 18,
                 pt_inflexion_grass = 1000,
              #   pt_inflexion_feu = 5,
...
#                   "outputs",
#                   "JAGS_outputs",
#                   paste0(name,"_complete_model3.RDS"))
#                 )
# }
...
for(i in 1:I){
  ...
  for (j in 1:J){
    ...
    grass_simu_i_j = mcmc_tot[j,"K_G_t_f_pluie_max"]*2*sigmo_pluie_simu_i_j*sigmo_coeff_competition_simu_i_j
...
summary(mcmc_tot[,"K_G_t_f_pluie_max"])
```
